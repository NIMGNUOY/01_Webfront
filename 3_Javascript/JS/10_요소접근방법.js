...
function classTest() {
    ...
    // divs 0, 1, 2 번째 요소에 접근해서 배경색 변경

    divs[0].style.backgroundColor = "#ddd"; 
    divs[1].style.backgroundColor = "aqua"; 
    divs[2].style.backgroundColor = "yellowgreen"; 
}
...
function tagNameTest() {
    ...
    for (let i = 0; i < tagList.length; i++) {  // 순차접근
        ...
        // tagList[i].innerText = "abc" : 내용으로 "abc"를 대입
        // tagList[i].innerText         : HTML에 작성된 내용을 얻어오기
...
function nameTest() {
    ...
    for (let i = 0; i < hobbyList.length; i++) {
        ...
        // input요소.checked -> 요소가 체크되어 있으면 true
        //                            체크되어 있지 않으면 false
...
function cssTest() {
    ...
    */
    ...
    // 첫번째 자식 div 접근해서 내용을 "CSS 선택자로 선택해서 값 변경됨"
...
function readValue() {
    ...
    if (input.value.trim().length == 0) {
        alert("채팅 내용을 입력해주세요.")
        input.value = "";   // 이전에 input 에 작성된 값 삭제

        input.focus(); // input 창에 커서가 활성화
    ...
    input.value = "";  // 이전 입력값 제거

    // bg.scrollHeight    :  bg 의 스크롤 전체 높이
...
/* 
    아이디가 user-input 인 요소에서 키가 올라오는 동작이
    발생했을 때(감지되었을 때) 올라온 키가 "Enter"키라면
    readValue() 함수를 호출
*/
